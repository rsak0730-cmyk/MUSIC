...
// Safely iterate and broadcast to all peers in a specific room
    ...
    ws.on('message', (message) => {
        ...
        if (data.type === 'HOST_ROOM') {
            ...
            if (currentRoom && rooms[currentRoom]) {
                ...
                // Security Check: Only the host can dictate playback states
    ...
    ws.on('close', () => {
        if (currentRoom && rooms[currentRoom]) {
            ...
            if (room.host === ws) {
                room.host = null; 
                setTimeout(() => {
                    // Destroy room if host doesn't reconnect within 10 seconds
