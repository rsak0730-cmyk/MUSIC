...
function broadcastPeerList(roomCode) {
    ...
    const msg = JSON.stringify({ type: 'PEER_LIST', peers: peersData, count: peersData.length });
    
    targets.forEach(client => client.send(msg));
}
    ...
    ws.on('message', (message) => {
        ...
        if (data.type === 'HOST_ROOM') {
            ...
            if (currentRoom && rooms[currentRoom]) {
                ...
                // STRICT SECURITY: Only the real host can control playback
...
console.log('SyncBeat Host-Controlled Backend Running...');
