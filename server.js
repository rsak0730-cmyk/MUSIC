...
wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
        ...
        if (msg.type === 'JOIN_ROOM') {
            ...
            if (rooms[roomCode].currentMedia) {
                ws.send(JSON.stringify({
                    type: 'PLAY_YOUTUBE',
                    ytId: rooms[roomCode].currentMedia.ytId || rooms[roomCode].currentMedia.videoId,
        ...
        // Host Media Relays & Cloud-Synced Seek Requests
        if (msg.type === 'PLAY_YOUTUBE' || msg.type === 'LOAD_VIDEO' || msg.type === 'MEDIA_SYNC' || msg.type === 'CONTROL') {
            if (!boundRoom || !rooms[boundRoom] || rooms[boundRoom].host !== ws) return;
            if (msg.type === 'PLAY_YOUTUBE' || msg.type === 'LOAD_VIDEO') rooms[boundRoom].currentMedia = msg;
            
            // Broadcasts to all connected devices, including the host
            broadcastToRoom(boundRoom, msg);
            return;
        }

        // Cloud-backed timeline scrubbing sync for all devices including host
        if (msg.type === 'SEEK_TIME') {
            if (!boundRoom || !rooms[boundRoom] || rooms[boundRoom].host !== ws) return;
            if (rooms[boundRoom].currentMedia) {
                rooms[boundRoom].currentMedia.time = msg.time;
            }
            broadcastToRoom(boundRoom, { type: 'SEEK_TIME', time: msg.time }); 
            return;
        }
        ...
        if (msg.type === 'REACTION') {
            if (!boundRoom || !rooms[boundRoom]) return;
            broadcastToRoom(boundRoom, { type: 'REACTION', emoji: msg.emoji }, ws);
            return;
        }
