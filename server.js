    ...
    ws.on('message', (message) => {
        ...
        if (data.type === 'HOST_ROOM') {
            ...
            if (currentRoom && rooms[currentRoom]) {
                ...
                // Restrict playback control, volume, and sync stream authority strictly to the host
                const hostOnlyTypes = ['PLAY_YOUTUBE', 'PLAY_YOUTUBE_PLAYLIST', 'CONTROL', 'VOLUME', 'SYNC_TIME'];
                if (hostOnlyTypes.includes(data.type) && !isSenderHost) {
                    return; 
                }
